...
def get_conn():
    ...
    r"SERVER=DESKTOP-C5AKQU5\SQLEXPRESS;"
    r"DATABASE=phonepe;"
...
# =========================================================
    ...
    if case.startswith("1."):
        ...
        states_df = pd.read_sql("SELECT DISTINCT state FROM dbo.agg_trans", conn)
        ...
        Years_df = pd.read_sql("SELECT DISTINCT Year FROM dbo.agg_trans ORDER BY Year", conn)
...
# --- query (you already have conn) ---
    ...
    # =====================================================
        ...
        years = ["All"] + sorted(pd.read_sql("SELECT DISTINCT Year FROM dbo.agg_trans", conn)['Year'].astype(int).tolist())
        sel_year = st.sidebar.selectbox("Year", years, index=0)
        ...
        if mode == "Year":
            ...
            q_pivot = """SELECT Year, state, SUM(Transaction_amount) AS total_amount FROM dbo.agg_trans GROUP BY Year,state"""
            ...
            q_top = f"""SELECT TOP {top_n} state AS name, SUM(Transaction_amount) AS total_amount
                        FROM dbo.top_trans_state
                        {f"WHERE Year={sel_year}" if sel_year!="All" else ""}
                        GROUP BY state ORDER BY total_amount DESC"""
            # removed state filter from pivot query
            q_pivot = f"""SELECT Year, state, SUM(Transaction_amount) AS total_amount FROM dbo.agg_trans
                        {f"WHERE Year={sel_year}" if sel_year!="All" else ""}
                        GROUP BY Year,state"""
            df_top = pd.read_sql(q_top, conn)
            df_line = pd.read_sql(q_pivot, conn).groupby('Year', as_index=False).total_amount.sum()
            ...
            q_top = f"""SELECT TOP {top_n} District AS name, SUM(Transaction_amount) AS total_amount
                        FROM dbo.top_trans_district
                        {f"WHERE Year={sel_year}" if sel_year!="All" else ""}
                        GROUP BY District ORDER BY total_amount DESC"""
            # removed dependency on sel_state; only filter by Year when provided
            q_pivot = f"""SELECT Year, SUM(Transaction_amount) AS total_amount FROM dbo.top_trans_district
                        {f"WHERE Year={sel_year}" if sel_year!="All" else ""}
                        GROUP BY Year"""
            df_top = pd.read_sql(q_top, conn)
            df_line = pd.read_sql(q_pivot, conn).groupby('Year', as_index=False).total_amount.sum()
            ...
            q_top = f"""SELECT TOP {top_n} Pincode AS name, SUM(Transaction_amount) AS total_amount
                        FROM dbo.top_trans_pincode
                        {f"WHERE Year={sel_year}" if sel_year!="All" else ""}
                        GROUP BY Pincode ORDER BY total_amount DESC"""
            q_pivot = f"""SELECT Year, SUM(Transaction_amount) AS total_amount FROM dbo.top_trans_pincode
